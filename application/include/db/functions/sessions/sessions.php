...
function remove(int $user_id) {
  ...
  // \db\insert("DELETE FROM `users_sessions` WHERE `user_id` = ? AND `session_start` < NOW() - INTERVAL 48 HOUR", [$user_id]);
  return \db\insert("DELETE FROM `users_sessions` WHERE `user_id` = ?", [$user_id]);
}
...
function create(int $user_id) {
  \db\insert("INSERT INTO `users_sessions` (`user_id`) VALUES (?)", [$user_id]);
  $uuid = select($user_id);
  return $uuid;
}
